    ...
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="space-y-4">
        {filteredHomework.map((homework) => (
          <Card key={homework.id} className="border-0 shadow-soft hover:shadow-medium transition-all duration-300">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <div className="flex items-center space-x-3 mb-2">
                    <BookOpen className="h-5 w-5 text-primary" />
                    <h3 className="text-lg font-semibold text-foreground">{homework.title}</h3>
                    <Badge className={`${getPriorityColor(homework.priority)} border`}>
                      {homework.priority}
                    </Badge>
                  </div>
                  ...
                  {homework.instructions && (
                    <div className="bg-muted/50 p-3 rounded-lg mb-4">
                      <p className="text-sm"><strong>Instructions:</strong> {homework.instructions}</p>
                    </div>
                  )}
                </div>
                
                <div className="text-right">
                  <Badge className={`${getStatusColor(homework.status)} border mb-2`}>
                  ...
                  <div className="text-sm text-muted-foreground">
                    ...
                    {homework.status === 'pending' && (
                      <p className={`font-medium ${getDaysUntilDue(homework.dueDate) < 0 ? 'text-red-600' : getDaysUntilDue(homework.dueDate) <= 1 ? 'text-yellow-600' : 'text-green-600'}`}>
                  ...
                  <div className="space-y-4">
                    {homework.submissionType === 'text' ? (
                      ...
                      <div className="border-2 border-dashed border-border rounded-lg p-8 text-center">
                        ...
                        <p className="text-sm text-muted-foreground">Supported formats: PDF, DOC, DOCX, JPG, PNG</p>
                      </div>
                    )}
                ...
                <div className="border-t pt-4">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    ...
                    <p className="text-green-700 text-sm">
                      Submitted on {format(homework.submittedDate!, 'MMM dd, yyyy')}
                    </p>
                    {homework.grade && (
                      <p className="text-green-700 text-sm mt-1">
                        Grade: {homework.grade}/{homework.maxMarks} ({((homework.grade / homework.maxMarks) * 100).toFixed(1)}%)
                      </p>
                    )}
                  </div>
                </div>
